...
% TODO: feet items?
\pingu@create@extra{flipflop}{%
    flip-flops/.code    	 = {\pingu@set@extra{flipflop}{true}\pingu@color{flipflop}{#1}},
    flip-flops/.default   = green% pingu@blue,%
    % flipflop band/.code  = \pingu@color{flipflop@band}{#1},%
    % flipflop shade/.code  = \pingu@color{flipflop@shade}{#1},%
    % flipflop shade width/.code  = \def\pingu@flipflop@shade@w{#1},%
    % flipflop text/.code  = \def\pingu@x@flipflop@text{#1},%
    % flipflop text style/.style = {\pingu@color@flipflop!65!pingu@white},
}{}

% TODO: needs one type of feet
\def\@pingu@x@flipflop@path{([xshift=-3.75mm,yshift=3mm]\pingu@name-foot-right) to[out=205,in=180] coordinate[pos=.84] (@a) ++(3.5mm,-2*\pingu@foot@single@h-1.75mm) coordinate (@c) to[out=0,in=285] coordinate[pos=.575] (@b) ++(2.5mm,2*\pingu@foot@single@h+1mm)}
\def\pingu@draw@eb@flipflop{%
\scope[rotate around={-\basicfeetbend+10:(\pingu@name-foot-right)}] % TODO
\pgfonlayer{very-background}
  \pingu@block{\pingu@color@flipflop} \@pingu@x@flipflop@path;
  \endpgfonlayer
\clip\@pingu@x@flipflop@path;
  \draw[semithick] ([xshift=-1mm]@a) to[bend left=15] coordinate[pos=.4] (@d) ([xshift=1mm]@b);
  \draw[semithick] (@d) to[bend right=5] (@c);
\endscope
}