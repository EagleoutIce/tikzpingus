...
\pingu@create@wing@extra{magnifier}{%
    ...
    magnifier \@lr/.default           = pingu@silver,
    ...
    magnifier \@lr\space width/.code  = \expandafter\def\csname pingu@magnifier@\@lr @width\endcsname{##1},
    magnifier \@lr\space height/.code  = \expandafter\def\csname pingu@magnifier@\@lr @height\endcsname{##1},
...
}{magnifier \@lr\space handle length=9mm, magnifier \@lr\space handle=pingu@bronze, magnifier \@lr\space handle shift=-4mm, magnifier \@lr\space width=7mm,magnifier \@lr\space height=3.5mm, magnifier \@lr\space xshift=-0.5*\csname pingu@magnifier@\@lr @width\endcsname,magnifier \@lr\space yshift=-1mm}
\def\pingu@draw@wi@magnifier#1{%
    \pingu@setup@wing{magnifier}{#1}%
  \scope[rotate around={\@angle:(\pingu@name-wing-#1)}]
% \csname pingu@color@magnifier#1\endcsname
    \pingu@block{\csname pingu@color@magnifier@#1@handle\endcsname}([xshift=\if@pingu@wing@item@side@left@0mm\else-1.5mm\fi]\pingu@name-wing-#1)++(\@sidesign*-.2mm,\csname pingu@magnifier@#1@shift\endcsname+4mm) coordinate (@)
    arc (250:-70:3mm) to[out=230,in=90] ++(-1mm,-3.5mm) arc(360:180:.5mm) to[out=90,in=-60] cycle ([x]@) circle[radius=2mm]; % arc (-70:-250:.33mm) arc (-70:250:3mm-.66mm);
    % ++(0,-\csname pingu@magnifier@#1@length\endcsname) arc(360:180:\@sidesign*.4mm and .3mm) coordinate[pos=.5] (\pingu@name-magnifier-#1-top) -- ++(0,\csname pingu@magnifier@#1@length\endcsname); % arc(360:180:\@sidesign*.4mm and .3mm) -- cycle;
    % \pingu@block[rounded corners=2pt]{\csname pingu@color@magnifier#1\endcsname} (\pingu@name-magnifier-#1-top)++(\csname pingu@magnifier@#1@xshift\endcsname,\csname pingu@magnifier@#1@yshift\endcsname) rectangle ++(\csname pingu@magnifier@#1@width\endcsname,\csname pingu@magnifier@#1@height\endcsname);
\endscope
}