...
% i know this is inefficient, but atm this is of no greater
...
% so that i can reuse them with the on-horse-extra
\def\pingu@x@HorseKeys#1{% useless arg
...
horse \@lr\space base shade/.code      = \pingu@coitlor{horse\@lr @base@shade}{##1},%
...
\newif\if@pingu@x@horse@below@flip@
\newif\if@pingu@x@horse@below@base@
% now we manually inject the creation of a 'below' extra:

\def\pingu@x@createBelowHorse#1#2#3{%
\def\@lr{below}%
\protected@edef\@tmp{\expandafter\expandafter\expandafter\expandafter\expandafter\unexpanded{{#1below}{#2}{#3}}}%
\expandafter\pingu@create@extra\@tmp}

% TODO: add to the extra drawers
\pingu@x@createBelowHorse{horse}{\pingu@x@HorseKeys{}}{\pingu@x@HorseDefaults,horse \@lr\space flip=false}

\def\pingu@draw@eb@horse{%
    % \pingu@setup@wing{horse}{below}% TODO: kill wing stuff?
% \scope[scale=\pingu@@horse@scale,every path/.append style={line cap=round, line join=round},shift=(\pingu@name-wing-#1)]
\csname if@pingu@x@horse@below@flip@\endcsname
\scope[xshift=-4cm-\csname pingu@x@horsebelow@xshift\endcsname,yshift=-42mm+\csname pingu@x@horsebelow@yshift\endcsname]
\else
\scope[xshift=4cm+\csname pingu@x@horsebelow@xshift\endcsname,yshift=-42mm+\csname pingu@x@horsebelow@yshift\endcsname,xscale=-1]
\fi
    \pingu@@drawer@horse{below}
\endscope
%\endscope
}