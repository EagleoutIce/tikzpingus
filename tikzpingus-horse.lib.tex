...
\pingu@block[draw=\csname pingu@color@horse#1@draw\endcsname,line width=1.75pt*\pingu@@horse@scale]{b!91!white} [rounded corners=1*\pingu@@horse@scale](@back) to[out=215,in=90] ++(-.4,-2.5) to[out=270,in=92] ++(0,-2.8) [sharp corners] to[bend right=20] ++(.6,0) to[bend right=10] cycle;

% back ear
\pingu@block[draw=\csname pingu@color@horse#1@draw\endcsname,line width=1.35pt*\pingu@@horse@scale,line join=bevel]{\csname pingu@color@horse#1\endcsname} ([xshift=.2mm,yshift=-1.5mm]@ear) to[bend left=20] ++(.373,.565) coordinate(@) to[bend left=10] ++(-.15,-.75);
...
\pingu@draw[line width=1.75pt*\pingu@@horse@scale]{\csname pingu@color@horse#1@draw\endcsname} ([yshift=-.33mm]@mouth) -- ++(-.035,.225);

\pingu@draw[line width=1.75pt*\pingu@@horse@scale,line join=round]{\csname pingu@color@horse#1@draw\endcsname} ([xshift=3.85mm,yshift=1.55mm]@mouth) to[out=140,in=10] ++(-.17,-.074) to[bend right=49] cycle;
...
\pingu@block[draw=\csname pingu@color@horse#1@draw\endcsname,line width=1.75pt*\pingu@@horse@scale]{b!91!white} [rounded corners=1*\pingu@@horse@scale](@hair) to[out=34,in=185] ++(3,1.3) to[bend left=10] ++(.415,-.1) -- ++(-.1,-.4) to[out=194,in=30] ([yshift=-11.5mm,xshift=1.5mm]@hair) -- cycle;

\pingu@block[draw=\csname pingu@color@horse#1@draw\endcsname,line width=1.35pt*\pingu@@horse@scale]{b!91!white} [rounded corners=1*\pingu@@horse@scale]([xshift=2mm,yshift=-1mm]@ear) -- ++(-.1,-.3275) to[bend left=4] ++(.475,-.175) to[bend right=9] cycle;

% ear
\pingu@block[draw=\csname pingu@color@horse#1@draw\endcsname,line width=1.35pt*\pingu@@horse@scale,line join=bevel]{\csname pingu@color@horse#1\endcsname} ([xshift=-.66mm,yshift=-1mm]@ear) to[bend left=20] ++(.373,.585) coordinate(@) to[bend right=10] ++(-.15,-.7);
\pingu@block[draw=\csname pingu@color@horse#1@draw\endcsname,line width=1.35pt*\pingu@@horse@scale,line join=bevel]{\csname pingu@color@horse#1\endcsname} (@) to[bend right=10] ++(-.15,-.7) to[bend right=8] cycle;


% the eye; this is no error it has draw as fill
\pingu@block{\csname pingu@color@horse#1@draw\endcsname} [rounded corners=.25*\pingu@@horse@scale] ([xshift=3.25mm,yshift=-7.5mm]@ear) to[bend right=20] ++(.125,-.2) to[bend right=50] ++(.1,.125) to[bend left=15] cycle;
}
...
\pingu@create@wing@extra{horse}{%
    ...
    % todo: farbe für mähne anpassen
}{horse \@lr\space draw=\csname pingu@color@horse\@lr\endcsname!82!black}
