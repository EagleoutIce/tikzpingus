...
\lstdefinelanguage{pingulang}{
	...
	alsoletter={.-!},
	moreemph=[2]{left,right,wing,eye,wings,tie,none,eyes,shiny,wink,wave,grab,hug,wave,cup,straw,xshift,yshift,meta,dots,name,heart,shock,bill,hair,hairstyle,feet,foot,simple,flat,angry,back,devil,normal},
...
\begin{tikzpicture}%
	...
	\foreach \c/\a in {wing-right/left,wing-right-low/left,wing-right-up/above} {
		...
		\node[\a=.5mm,font=\ttfamily,scale=.35,inner sep=2.5pt] (expl-\c) at (pingu-\c) {\c};
...
\begin{center}
	\null\hfill\parbox[c]{2\wd\pinguwingright}{\scalebox2{\usebox\pinguwingright}}\hfill\parbox[c]{4cm}{\centering\scriptsize\color{gray}\sffamily And yes, the wings are deliberately placed asymmetrical.\endgraf}\hfill
	\parbox[c]{2\wd\pinguwingleft}{\scalebox2{\usebox\pinguwingleft}}\hfill\null
\end{center}
...
While the penguin options offer modification of basically every drawing routine (through other styles like |@block|), it is tedious to change them every time.
...
\begin{itemize}
	\foreach \tx/\s in {{draw everything as a line}/{pingu line}, {draws every component with transparency}/{pingu ghost parts}, {draws every layer with transparency}/{pingu ghost}, {set all of the \say{devil}-components}/{pingu devil},{shows the penguin from the back (left and right are flipped!)}/{pingu back}} {
		\item \texttt{\s}: \tx.\smallskip\\*
		\null\qquad\scalebox{.5}{
			\begin{tikzpicture}
				\pingu[\s]
			\end{tikzpicture}
		}
