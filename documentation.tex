...
\def\explaincolor{pingu@purple!6!white}
\newenvironment{keyexplain}[4][/pingu/]{\begin{minipage}{\linewidth}%
	\parskip\medskipamount
	\phantomsection\label{pk:#1#2}\index{#2?\hyperref[pk:#1#2]{\lpingu{#2}}}%
	...
	\hspace*{-2em}\colorbox{\explaincolor}{%
		\parbox{\dimexpr\linewidth-2\fboxsep+2em}{%
			\lpingu{#1#2 =\ }\texttt{<\textit{#3}>}\hfill
				\ifx\@tmp\@empty\else\textcolor{gray}{(}\pgfkeys{/pingu/.cd,defaults}#4\textcolor{gray}{)}\fi%
		}
	}\\*[\smallskipamount]}
{\end{minipage}\bigskip\par}

\def\singleshortcut#1#2#3#4{\begin{keyexplain}[#1]{#2 #3}{#4}{\csname pingudefa#2\endcsname}%
	This is a shortcut for: \texttt{\keyref[#1]{#2} = {\optstyle#3}}.
\bgroup
...
\newcommand{\keyalias}[3][/pingu/]{\begin{keyexplain}[#1]{#2}{\csname pinguopt#3\endcsname}{\csname pingudefa#3\endcsname}%
	Alias for \keyref[#1]{#3}.
\end{keyexplain}}
...
\shortcuts{left foot}{none,normal,simple,back}{color}
...
\shortcuts{right foot}{none,normal,simple,back}{color}
...
\shortcuts{feet}{none,normal,simple,back}{color}
...
% TODO: all the shortcuts like eyes wink
