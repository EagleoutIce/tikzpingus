...
\addtokomafont{title}{\@declaredcolor{pingu@purple}}
\addtokomafont{author}{\normalsize}
\addtokomafont{date}{\normalsize}

\def\optstyle{\@declaredcolor{pingu@blue!75!black}\slshape}
\def\lstfnsize{-.7}
\lstdefinestyle{lstpingu}{%
	...
	emphstyle=[3]\@declaredcolor{gray!75!white},
	emphstyle=[4]\@declaredcolor{pingu@blue!40!black},
	texcsstyle=*\@declaredcolor{gray}\bfseries,
	texcsstyle=*[2]\@declaredcolor{pingu@purple}\bfseries,
	texcsstyle=*[3]\@declaredcolor{gray!60!white},
	lineskip=2.5pt,
	keepspaces=true,
	moredelim=[s][\itshape]{<}{>},
}
...
\AtEndDocument{%
	...
	\immediate\write\@auxout{%
		...
		\noexpand\gdef\noexpand\allmain@cmds{\allmain@cmds}%
	}%
}

\def\CreateOverview#1{%
	\@for\@elem:=#1\do{\ifx\@elem\@empty\else
		% we want to create a preview for that :D --lets see
		%\edef\@tmp{\noexpand\@CreateCodeHyperLink{#2}{\@sub}{pk:/pingu/\@elem}}
		\expandafter\tikz\expandafter{\expandafter\pingu\expandafter[\@elem]}%
	\fi}
}

\let\allmain@cmds\@empty
\AtBeginDocument{%
...
\expandafter\CreateOverview\expandafter{\allmain@cmds}%
\let\allmain@cmds\@empty
}
...
\newenvironment{keyexplain}[4][/pingu/]{\minipage\linewidth
	...
	\if@pingu@hidefrompreview@\global\@pingu@hidefrompreview@false\else
		\gappto\allmain@cmds{#2,}% appto, but do not expand
	\fi
	% update the range
...
\newif\if@pingu@hidefrompreview@
\newenvironment{subkeyexplain}[5][/pingu/]{%
\begingroup
\@pingu@hidefrompreview@true
...
\begin{tcblisting}{sidebyside=false, title=Bodyparts-Showcase,externalize listing=bodyparts-showcase}
\begin{tikzpicture}
...
The first line in the example sets the seed: % we do not externalize this one
...
\keyexplain{devil wings}{color}{pingu@black}
\begin{tcblisting}{@}
\begin{tikzpicture}
...
\keyexplain{construction helmet}{color}{pingu@yellow}
\begin{tcblisting}{@}
\begin{tikzpicture}
...
\keyexplain{conical hat}{color}{pingu@yellow}
\begin{tcblisting}{@}
\begin{tikzpicture}
...
\keyexplain{laptop left}{color}{gray!80!pingu@white}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[laptop left]
\end{tikzpicture}
...
\keyexplain{laptop right}{color}{gray!80!pingu@white}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[laptop right]
\end{tikzpicture}
...
\keyexplain{horse left}{color}{pingu@bronze!80!pingu@black}
	Give it a horse:
\begin{tcblisting}{@}
\begin{tikzpicture}
...
\keyexplain{horse right}{color}{pingu@bronze!80!pingu@black}
	Give it a horse:
\begin{tcblisting}{@}
\begin{tikzpicture}
