...
\newenvironment{keyexplain}[4][/pingu/]{\minipage{\linewidth}
	...
	\hspace*{-\keyexplainindent}\ifcsname pingu@@lib@#2@\endcsname\index{Library!\textit{\csname pingu@@lib@#2@\endcsname}!#2?\hyperref[pk:#1#2]{\protect\lpingu{#2}}}\llap{$\underset{\text{\tiny\textsf{Library}}}{\text{\textit{\footnotesize\strut\csname pingu@@lib@#2@\endcsname}}}$\quad~}\fi\colorbox{\explaincolor}{%
...
\index{Library}I've split the penguin features into a set of libraries. While all of them are loaded by default, the |bare| package-option disables the automatic loading of all libraries. They can be loaded (locally to the current group) using |\pinguloadlibrary| and |\pinguloadlibraries| passing on a comma separated list of desired libraries.
See the full reference to learn which key comes from which library.
...
\def\percenttargetreductionpengus{75}\def\lstfnsize{-1.65}
\tcbset{%
