...
\newenvironment{keyexplain}[4][/pingu/]{\minipage{\linewidth}
	...
	{\def\\{}\protected@xdef\@idxkey{#2}}%% backslash should not interfere with keys
	\tcbset{@/.style={externalize listing=key-\keyexternalname}}% set for externalize
	\phantomsection\label{pk:#1\@labelhack\@key}\index{\cursub\@idxkey?\hyperref[pk:#1\@key]{\protect\lpingu{#2}}}%
	% get its length:
	\StrLen{#2}[\@cclen]%
...
\begin{commandexplain}{pingu@create@selection}{\mand{name}\mand{none default}}
	...
	The \textit{none default} is used for the default member of the selection: \textit{none} which is active, whenever no other key is selected by \cmdref{pingu@<name>@select}.
	You can add keys by using \cmdref{pingu@<name>@add} and execute the selection by \cmdref{@pingu@drawer@<name>@}.
\end{commandexplain}
TODO: subcommandexplain and better cmdref

\subsection{Adding new Eyes}\label{internal:eyes}
