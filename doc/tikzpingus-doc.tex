...
\usepackage[linkcolor=doc-main,urlcolor=doc-main,colorlinks,breaklinks,pdfusetitle]{hyperref}
...
\def\explaincolor{doc-main!8!white}
\def\cursub{}
...
\def\PenguinTitle{%
\setbox0=\hbox{\tikz{\pingu[body type=tilt-right,eyes wink,conical hat,cane right,bow tie=pingu@purple,left wing grab,cup=pingu@purple!82!pingu@black]}}%
...
\begin{tikzpicture}
    \node[text width=.95\linewidth, inner sep=11pt] (m) {%
	 ...
	 I would be happy for any feedback or issues on the \href{https://github.com/EagleoutIce/tikzpingus}{tikzpingus}-GitHub. \textbf{TODO: move down the right penguin :C he looks so sad}
    };
...
At the moment, it loads \href{https://www.ctan.org/pkg/pgf}{\TikZ}, which loads a lot of other packages (e.g. |xcolor|), and \href{https://www.ctan.org/pkg/etoolbox}{etoolbox}.
...
There are \textit{a lot} of configuration-options which can be passed as an optional argument via the known |<key>=<value>|-style. See \autoref{Gadget-Overview} for a gadget overview.
...
\end{tikzpicture}
\end{tcblisting}
\newpage Lets look at those coordinates in more detail (all labels are to be prefixed by |<pingu-name>-|):
