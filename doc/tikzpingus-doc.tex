...
%
% This work consists of the files tikzpingus.sty and tikzpingus-doc.tex
...
\def\@pingu@stripquotes#1"#2"\@nil{\def\@ts@val{#2}}
\def\RawSelectionShowcase#1#2{\scalebox{.62}{%
\tikzpicture[baseline={(pingu-bottom-center)}]
\pingu[:mix=30!white,name=pingu,#1=#2]
\pgfonlayer{foreground}
\node[below=3mm] at(pingu-bottom-center) {\strut};
\pgfinterruptboundingbox
\node[below=3mm,fill=white,fill opacity=.52,text opacity=1,rounded corners=3pt] at(pingu-bottom-center) {\textsf{\strut\smash{\ifstrequal{#2}{normal}{\textbf{#2}}{#2}}}};
\endpgfinterruptboundingbox
\endpgfonlayer
\endtikzpicture
}}

% selection | key to set | key to clear
\newcommand*\TypesetSelection[3]{%
\global\@tempcnta\z@
\begingroup\parskip\z@ \parindent\z@
\edef\@ts@list{\csname @pingu@#1@\endcsname}%
\@for\@ts@elem:=\@ts@list\do{%
  \advance\@tempcnta\@ne
  \ifnum\@tempcnta>6 \global\@tempcnta\@ne\vspace{-3pt}\par\fi
  \ifx\@ts@elem\@empty\else
  \expandafter\@pingu@stripquotes\@ts@elem\@nil
  \edef\@tmp{\noexpand\RawSelectionShowcase{/pingu/#3/.code={},#2}{\@ts@val}}%
  \hfill\parbox[b][\pingu@showcase@height]{.137\linewidth}{\centering~\clap{\@tmp}~}\hfill
  \fi
}\strut%
\endgroup}

\def\@ts@anchordot#1#2#3#4{%
	\path[fill=teal] ({#1},{#2}) circle [radius=1.125\p@];
	\node[#4=.4mm,font=\ttfamily,scale=.45,inner sep=1.25pt,
	      fill=white,fill opacity=.75,text opacity=1] (@a-#3) at ({#1},{#2}) {#3};
	\draw[teal,thin] (@a-#3) -- ({#1},{#2});%
}
\newcommand*\TypesetAnchors{%
\begin{tikzpicture}
	\pingu[eyes shiny]
	% \pingu keeps its state to itself, so re-establish what the anchors read
	\pgfqkeys{/pingu}{defaults}\pingu@bbox@update
	\pgfonlayer{foreground}
	\foreach \@ts@a/\@ts@d in {origin/left,center/right,north/above,south/below,
		east/right,west/below left,north east/above right,north west/above left,
		south east/below right,south west/below left,head/left,bill/right,
		belly/left,left shoulder/right,right shoulder/above left,
		left wing/right,right wing/left,
		left wing tip/below right,right wing tip/below left}{%
		\expandafter\expandafter\expandafter\@ts@anchordot
			\csname @pingu@anchor@\@ts@a @\endcsname{\@ts@a}{\@ts@d}%
	}
	\endpgfonlayer
\end{tikzpicture}}

% <compact toc>
% A run-in table of contents: the entries end up as one dot-separated
% paragraph instead of a page holding a single entry per line.
\newcommand*\@ctoc@sep{\hspace{.45em}\textcolor{@softgray@b}{\textbullet}\hspace{.45em}}
\newif\if@ctoc@first
\newcommand*\@ctoc@entry[2]{%
	\if@ctoc@first\@ctoc@firstfalse\else\@ctoc@sep\fi
	\mbox{#1\,\textcolor{@softgray@b}{#2}}%
}
\newcommand\CompactTableOfContents{%
	\begingroup
		\@ctoc@firsttrue
		\renewcommand*\numberline[1]{##1\nobreakspace}%
		\let\l@section\@ctoc@entry
...
\def\RawShowcase#1#2{\scalebox{.62}{%\begin{tcbexternal}{showcase-#2#1}%
\tikzpicture[baseline={(pingu-bottom-center)}]
\pingu[:mix=30!white,name=pingu,#1#2]
\pgfonlayer{foreground}
...
\@for\@elem:=\pengu@all@showcases\do{%
	\advance\@tempcnta\@ne
	\ifnum\@tempcnta>6 \global\@tempcnta\@ne\par\fi
	\ifx\@elem\@empty\else
	% space to space cmd
...
\date{Version v1.4 \textendash{} 2026/02/27}
...
\clearpage
\phantomsection\addcontentsline{toc}{section}{Overviews}

\foreach \selection/\name/\clear/\txt in {%
	bodytype/body type/@body/{See \autoref{mrk:pengu-change-comps} for more info.},
	lefteye/eyes/@eyes/{See \autoref{mrk:pengu-eye} for more info. You can also set eyes individually with \keyref{left eye} and \keyref{right eye}.},
	leftwing/wings/@wings/{See \autoref{subsec:wings} for more info. You can also set wings individually with \keyref{left wing} and \keyref{right wing}.},
	leftfoot/feet/@feet/{See \autoref{mrk:pengu-change-comps} for more info. You can also set feet individually with \keyref{left foot} and \keyref{right foot}.},
	bill/bill/@bill/{See \autoref{mrk:pengu-change-comps} for more info.},
	hairstyle/hairstyle/@hair/{See \autoref{mrk:pengu-change-comps} for more info.}%
} {\typeout{Generating overview for \name}%
\needspace{6\baselineskip}\begin{center}
	\phantomsection\label{sec:overview-\name}
	{\large\textbf{All \textit{\name} Options}}\\
	\txt\par\noindent
	\TypesetSelection{\selection}{\name}{\clear}
\end{center}
\textcolor{gray}{\footnotesize Use \texttt{\name=\textit{<option>}} to set the \textit{\name} of a penguin.}
}
\typeout{Generating overview for anchors}%
\needspace{6\baselineskip}\begin{center}
	\phantomsection\label{sec:overview-anchors}
	{\large\textbf{All \textit{anchor} Options}}\\
	See \autoref{mrk:anchors} for more info. \textit{head top} and \textit{feet} are
	aliases of \textit{north} and \textit{south}; any other point works as well.\par\noindent
	\TypesetAnchors
\end{center}
\textcolor{gray}{\footnotesize Use \texttt{anchor=\textit{<option>}} to move that point of a penguin onto the origin.}

\phantomsection\label{Gadget-Overview}
\begin{center}
	{\large\textbf{Gadget Overview}}\\
	These automatically link to the respective documentation sections.
\end{center}
\TypesetShowcases
\clearpage
...
Copyright \textcopyright\ \textit{Florian Sihler}. Permission is granted to copy, distribute and\slash or modify this software under the terms of LPPL Version~1.3c (please note that this changed on 2026-02-27 from GPL~v3, versions \textbf{after}~v1.3 are licensed with LPPL~1.3c).\\*
...
There are \textit{a lot} of configuration-options which can be passed as an optional argument via the known |<key>=<value>|-style. See \autoref{Gadget-Overview} at the start for a complete gadget overview.
...
\subsection{Anchors and Placement}\label{mrk:anchors}
The origin of a penguin sits at its right shoulder. \keyref{anchor} moves the
penguin so that another of its points lands on the origin instead:
\begin{tcblisting}{title={Two anchors},externalize listing=anchor-pengu}
\begin{tikzpicture}
	\pingu[anchor=belly]
	\pingu[anchor=head,xshift=3cm]
	\draw[red] (-2.5mm,0) -- (2.5mm,0) (0,-2.5mm) -- (0,2.5mm)
	           ([xshift=3cm]-2.5mm,0) -- ([xshift=3cm]2.5mm,0)
	           ([xshift=3cm]0,-2.5mm) -- ([xshift=3cm]0,2.5mm);
\end{tikzpicture}
\end{tcblisting}
Besides the names listed at \keyref{anchor} any point works, e.g.\ |anchor={(1cm,-5mm)}|.

Anchors refer to the \textit{silhouette of the body}; gadgets, wings, and feet
may stick out of it, so the anchors do not change with the extras a penguin
wears. Its nine outer anchors are coordinates as well (prefixed by
|<|\keyref{name}|>-|): |center|, |north|, |south|, |east|, |west|,
|north east|, |north west|, |south east|, and |south west|. The anchor in use
is available as |<|\keyref{name}|>-anchor| and, together with the silhouette,
shown by \keyref{meta-dots}.

Those outer anchors are what \keyref{right of}, \keyref{left of},
\keyref{above of}, and \keyref{below of} use, each taking the name of the other
penguin and, after a colon, the distance to keep:
\begin{tcblisting}{title={A row of penguins},externalize listing=relative-pengu}
\begin{tikzpicture}
	\pingu[name=a,wings wave]
	\pingu[name=b,right of=a:5mm,large size]
	\pingu[name=c,left of=a:5mm,small size]
	\pingu[name=d,above of=a:2mm,eyes shiny]
	\pingu[name=e,below of=a:2mm,body=pingu@blue]
\end{tikzpicture}
\end{tcblisting}
\keyref{right of} and \keyref{left of} keep both penguins on the same ground
line, \keyref{above of} and \keyref{below of} center them horizontally. An
\keyref{anchor} passed \textit{after} one of them keeps the target point but
changes which point of the new penguin is moved onto it.
...
\begin{tikzpicture}
	...
	\pingu[bill open, feet simple, heart=orange, xshift=7cm]
...
\section{A Collection of Examples}

To showcase the capabilities of this package, I have created a couple of example-penguins.

\begin{tcblisting}{title={The Waddlers!},externalize listing=musicians,nameref={Example: Musicians},label=example-musicians,float}
\begin{tikzpicture}
   ...
   \pingu[xshift=1.75cm,yshift=-3cm, right wing grab, eyes shiny,body type=legacy,small,shoes=pingu@purple,heart,wool hat=pingu@purple!80!black,bill=open,microphone right]
\end{tikzpicture}
\end{tcblisting}

\begin{tcblisting}{title={Lord Double-Dagger},externalize listing=double-dagger,nameref={Example: Double-Dagger},label=example-double-dagger,float}
\begin{tikzpicture}
...
\subsubsection{Anchors and Placement}

\keyexplain{anchor}{anchor or point}{origin}
	Move the penguin so that the given anchor ends up at the origin (see~\autoref{mrk:anchors}).
	Valid names are \textit{origin} (the right shoulder, the default),
	\textit{center}, \textit{north}, \textit{south}, \textit{east}, \textit{west},
	\textit{north east}, \textit{north west}, \textit{south east}, \textit{south west},
	\textit{head}, \textit{head top}, \textit{bill}, \textit{belly},
	\textit{feet}, \textit{left shoulder}, and \textit{right shoulder}.
	Anything else has to be a point |(<x>,<y>)| in the coordinate system of the penguin
...
\keyexplain{right of}{name:distance}{}
	Place the penguin right of the penguin with the given \keyref{name}, on the same ground line.
	Without a distance (|right of=a|) both penguins touch.
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[name=a]
	\pingu[right of=a:5mm,body=pingu@blue]
\end{tikzpicture}
\end{tcblisting}
\endkeyexplain

\keyexplain{left of}{name:distance}{}
	Like \keyref{right of}, but to the left.
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[name=a]
	\pingu[left of=a:5mm,body=pingu@blue]
\end{tikzpicture}
\end{tcblisting}
\endkeyexplain

\keyexplain{above of}{name:distance}{}
	Place the penguin above the penguin with the given \keyref{name}, centered on it.
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[name=a]
	\pingu[above of=a:5mm,body=pingu@blue]
\end{tikzpicture}
\end{tcblisting}
\endkeyexplain

\keyexplain{below of}{name:distance}{}
	Like \keyref{above of}, but below.
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[name=a]
	\pingu[below of=a:5mm,body=pingu@blue]
\end{tikzpicture}
\end{tcblisting}
\endkeyexplain
...
\keyexplain{bill above}{true/false}{\if@pingu@bill@above true\else false\fi}
Makes any bill appear on the foreground layer.
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[bill color=green,bill above]
\end{tikzpicture}
\end{tcblisting}
\endkeyexplain
...
\keyexplain{:mix}{color-mix}{50!white}
	Mix the main penguin with given colors by adapting the individual drawing colors individually, but leave all extras intact. This can be used (for example) to gray-out the penguin:
\begin{tcblisting}{@}
\begin{tikzpicture}
...
\subsubsection{The rectangular medal}
\showkeyexplain{rectmedal}{color}{pingu@yellow!70!pingu@bronze}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[rectmedal]
\end{tikzpicture}
...
\subkeyexplain{rectmedal}{rectmedal band}{color}{\pingu@color@rectmedal@band}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[rectmedal, rectmedal band=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

{\def\pingu@color@rectmedal{<rectmedal-color>}
\subkeyexplain{rectmedal}{rectmedal shade}{color}{\pingu@color@rectmedal@shade}
Change the color of the outer rectmedal ring:
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[rectmedal, rectmedal shade=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

\subkeyexplain{rectmedal}{rectmedal width}{length}{7.5mm}
Change the width of rectmedal:
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[rectmedal, rectmedal width=2mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{rectmedal}{rectmedal height}{length}{6.5mm}
Change the height of rectmedal:
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[rectmedal, rectmedal height=2mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{rectmedal}{rectmedal shade width}{length}{.75pt}
Change the width of the outer rectmedal ring:
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[rectmedal, rectmedal shade=green,
	       rectmedal shade width=2mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{rectmedal}{rectmedal text}{text}{\pingu@x@rectmedal@text}
Set the text displayed in the rectmedal. The style can be changed by
updating the substyle \texttt{rectmedal text style}.
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[rectmedal, rectmedal text=XY,
	      rectmedal text style/.style={black}]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\keyexplain{gold rectmedal}{text}{1}
Basically the same as the normal rectmedal. This will activate \keyref{rectmedal}:
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[gold rectmedal]
\end{tikzpicture}
\end{tcblisting}
\endkeyexplain

\keyexplain{silver rectmedal}{text}{2}
Basically the same as the normal rectmedal, but with a silver color. This will activate \keyref{rectmedal}:
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[silver rectmedal]
\end{tikzpicture}
\end{tcblisting}
\endkeyexplain

\keyexplain{bronze rectmedal}{text}{3}
Basically the same as the normal rectmedal, but with a bronze color. This will activate \keyref{rectmedal}:
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[bronze rectmedal]
\end{tikzpicture}
\end{tcblisting}
\endkeyexplain
...
\subkeyexplain{glasses}{glasses bend left}{angle}{32.5}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[glasses, glasses bend left=45]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{glasses}{glasses bend right}{angle}{32.5}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[glasses, glasses bend right=45]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{glasses}{glasses bend}{angle}{32.5}
Sets \keyref{glasses bend left} and \keyref{glasses bend right} with the same value.
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[glasses, glasses bend=45]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{glasses}{glasses upper bend}{angle}{8}
Sets the bending of the upper part of the glasses.
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[glasses, glasses upper bend=30]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain
...
\subsubsection{The bunny hears}

\showkeyexplain{bunny ears}{color}{brown!95!pingu@main}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[bunny ears=green]
\end{tikzpicture}
\end{tcblisting}
\endshowkeyexplain

\keyalias{bunnyears}{bunny ears}
\keyalias{bunny-ears}{bunny ears}

{\def\pingu@color@bunnyears{<bunny ears-color>}
\subkeyexplain{bunny ears}{bunny ears band}{color}{\pingu@color@bunnyears@band}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[bunny ears,bunny ears band=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain
\subkeyalias{bunnyears band}{bunny ears band}{bunny ears}
\subkeyalias{bunny-ears band}{bunny ears band}{bunny ears}
}

{\def\pingu@color@bunnyears{<bunny ears-color>}
\subkeyexplain{bunny ears}{bunny ears b}{color}{\pingu@color@bunnyears@b}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[bunny ears,bunny ears b=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyalias{bunnyears b}{bunny ears b}{bunny ears}
\subkeyalias{bunny-ears b}{bunny ears b}{bunny ears}
}
...
Disable or enable the glow of the halo. The glow is enabled by default, use the \texttt{noglows} package option to disable it.
\begin{tcblisting}{@}
\begin{tikzpicture}
...
\subsubsection{The detective cap}
\showkeyexplain{detective cap}{color}{pingu@bronze!60!lightgray!95!green}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[detective cap=green]
\end{tikzpicture}
\end{tcblisting}
\endshowkeyexplain

\subkeyexplain{detective cap}{detective cap padding}{length}{\pingu@x@detectivecap@padding}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[detective cap,
	detective cap padding=4mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{detective cap}{detective cap position}{angle>:(<x>,<y>)<scale}{\pingu@x@detectivecap@angle:(\pingu@x@detectivecap@xshift,\pingu@x@detectivecap@yshift)\{\pingu@x@detectivecap@scale\}}
Currently, this is a very cumbersome command to change various detective cap parameters at the same time:
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[detective cap,
		detective cap position={1:(-.1cm,-.275cm){1.33}}]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain
...
\showkeyexplain{drums}{color}{brown!10!pingu@white}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[drums=green]
\end{tikzpicture}
...
{\def\pingu@color@drums{<drums-color>}
\subkeyexplain{drums}{drums center band}{color}{\pingu@color@drums@band@center}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[drums,drums center band=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@drums{<drums-color>}
\subkeyexplain{drums}{drums center side}{color}{\pingu@color@drums@side@center}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[drums,drums center side=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@drums{<drums-color>}
\subkeyexplain{drums}{drums center text}{text}{\pingu@drums@text}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[drums,drums center text=Foo!]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@drums{<drums-color>}
\subkeyexplain{drums}{drums left base}{color}{\pingu@color@drums@base@left}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[drums,drums left base=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@drums{<drums-color>}
\subkeyexplain{drums}{drums left band}{color}{\pingu@color@drums@band@left}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[drums,drums left band=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@drums{<drums-color>}
\subkeyexplain{drums}{drums left side}{color}{\pingu@color@drums@side@left}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[drums,drums left side=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}
...
{\def\pingu@color@drums{<drums-color>}
\subkeyexplain{drums}{drums left color}{color}{\pingu@color@drums@left}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[drums,drums left color=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@drums{<drums-color>}
\subkeyexplain{drums}{drums right base}{color}{\pingu@color@drums@base@right}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[drums,drums right base=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@drums{<drums-color>}
\subkeyexplain{drums}{drums right band}{color}{\pingu@color@drums@band@right}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[drums,drums right band=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@drums{<drums-color>}
\subkeyexplain{drums}{drums right side}{color}{\pingu@color@drums@side@right}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[drums,drums right side=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}
...
{\def\pingu@color@drums{<drums-color>}
\subkeyexplain{drums}{drums right color}{color}{\pingu@color@drums@right}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[drums,drums right color=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}
...
\subsubsection{The basket}

\showkeyexplain{basket left}{color}{pingu@white!90!pingu@black}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[basket left=green]
\end{tikzpicture}
\end{tcblisting}
\endshowkeyexplain

{\def\pingu@color@basket@left{<basket left-color>}
\subkeyexplain{basket left}{basket left back}{color}{\pingu@color@basketleft@back}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[basket left, basket left back=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain
}

\subkeyexplain{basket left}{basket left egg a}{color}{\pingu@color@basketleft@egg@a}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[basket left, basket left egg a=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{basket left}{basket left egg b}{color}{\pingu@color@basketleft@egg@b}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[basket left, basket left egg b=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{basket left}{basket left egg c}{color}{\pingu@color@basketleft@egg@c}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[basket left, basket left egg c=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{basket left}{basket left xshift}{length}{\pingu@basket@left@xshift}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[basket left, basket left xshift=2mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{basket left}{basket left yshift}{length}{\pingu@basket@left@yshift}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[basket left, basket left yshift=2mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain


\showkeyexplain{basket right}{color}{pingu@white!90!pingu@black}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[basket right=green]
\end{tikzpicture}
\end{tcblisting}
\endshowkeyexplain

{\def\pingu@color@basket@right{<basket right-color>}
\subkeyexplain{basket right}{basket right back}{color}{\pingu@color@basketright@back}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[basket right, basket right back=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain
}

\subkeyexplain{basket right}{basket right egg a}{color}{\pingu@color@basketright@egg@a}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[basket right, basket right egg a=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{basket right}{basket right egg b}{color}{\pingu@color@basketright@egg@b}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[basket right, basket right egg b=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{basket right}{basket right egg c}{color}{\pingu@color@basketright@egg@c}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[basket right, basket right egg c=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{basket right}{basket right xshift}{length}{\pingu@basket@right@xshift}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[basket right, basket right xshift=2mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{basket right}{basket right yshift}{length}{\pingu@basket@right@yshift}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[basket right, basket right yshift=2mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain
...
\subsubsection{The flip flops}

\showkeyexplain{flip-flops}{color}{pingu@blue!70!pingu@white!60!cyan}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[flip-flops=green]
\end{tikzpicture}
\end{tcblisting}
\endshowkeyexplain

{\def\pingu@color@flipflop{<flip-flops>}
\subkeyexplain{flip-flops}{flip-flops base left}{color}{\pingu@color@flipflop@base@left}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[flip-flops, flip-flops base left=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@flipflop{<flip-flops>}
\subkeyexplain{flip-flops}{flip-flops base right}{color}{\pingu@color@flipflop@base@right}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[flip-flops, flip-flops base right=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@flipflop{<flip-flops>}
\subkeyexplain{flip-flops}{flip-flops band left}{color}{\pingu@color@flipflop@band@left}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[flip-flops, flip-flops band left=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@flipflop{<flip-flops>}
\subkeyexplain{flip-flops}{flip-flops band right}{color}{\pingu@color@flipflop@band@right}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[flip-flops, flip-flops band right=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@flipflop{<flip-flops>}
\subkeyexplain{flip-flops}{flip-flops color right}{color}{\pingu@color@flipflop@right}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[flip-flops, flip-flops color right=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

\subsubsection{The shoes}

\showkeyexplain{shoes}{color}{pingu@bronze!70!pingu@red!80!pingu@white!80!brown}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[shoes=green]
\end{tikzpicture}
...
{\def\pingu@color@shoes{<shoes>}
\subkeyexplain{shoes}{shoes base left}{color}{\pingu@color@shoes@base@left}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[shoes, shoes base left=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@shoes{<shoes>}
\subkeyexplain{shoes}{shoes base right}{color}{\pingu@color@shoes@base@right}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[shoes, shoes base right=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@shoes{<shoes>}
\subkeyexplain{shoes}{shoes color right}{color}{\pingu@color@shoes@right}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[shoes, shoes color right=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

\subsubsection{The sneaker}

\showkeyexplain{sneaker}{color}{pingu@bronze!70!pingu@red!80!pingu@white!80!brown}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[sneaker=green]
\end{tikzpicture}
...
{\def\pingu@color@sneaker{<sneaker>}
\subkeyexplain{sneaker}{sneaker base left}{color}{\pingu@color@sneaker@base@left}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[sneaker, sneaker base left=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@sneaker{<sneaker>}
\subkeyexplain{sneaker}{sneaker base right}{color}{\pingu@color@sneaker@base@right}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[sneaker, sneaker base right=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}
...
{\def\pingu@color@sneaker{<sneaker>}
\subkeyexplain{sneaker}{sneaker front left}{color}{\pingu@color@sneaker@front@left}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[sneaker, sneaker front left=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@sneaker{<sneaker>}
\subkeyexplain{sneaker}{sneaker front right}{color}{\pingu@color@sneaker@front@right}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[sneaker, sneaker front right=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@sneaker{<sneaker>}
\subkeyexplain{sneaker}{sneaker side left}{color}{\pingu@color@sneaker@side@left}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[sneaker, sneaker side left=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@sneaker{<sneaker>}
\subkeyexplain{sneaker}{sneaker side right}{color}{\pingu@color@sneaker@side@right}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[sneaker, sneaker side right=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@sneaker{<sneaker>}
\subkeyexplain{sneaker}{sneaker band left}{color}{\pingu@color@sneaker@band@left}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[sneaker, sneaker band left=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@sneaker{<sneaker>}
\subkeyexplain{sneaker}{sneaker band right}{color}{\pingu@color@sneaker@band@right}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[sneaker, sneaker band right=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@sneaker{<sneaker>}
\subkeyexplain{sneaker}{sneaker color right}{color}{\pingu@color@sneaker@right}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[sneaker, sneaker color right=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}
...
Toggle the glow of the lightsaber. The glow is enabled by default; use the \texttt{noglows} package option to disable it.
\begin{tcblisting}{}
\begin{tikzpicture}
...
\subsubsection{The guitar}

\showkeyexplain{guitar left}{color}{pingu@red!90!black}
\begin{tcblisting}{}
\begin{tikzpicture}
	\pingu[guitar left=green]
\end{tikzpicture}
\end{tcblisting}
\endshowkeyexplain

{\def\pingu@color@guitarleft{<guitar-left-color>}
\subkeyexplain{guitar left}{guitar left side color}{color}{\pingu@color@guitarleft@side}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[guitar left, guitar left side color=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@guitarleft{<guitar-left-color>}
\subkeyexplain{guitar left}{guitar left tint color}{color}{\pingu@color@guitarleft@tint}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[guitar left, guitar left tint color=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@guitarleft{<guitar-left-color>}
\subkeyexplain{guitar left}{guitar left outline color}{color}{\pingu@color@guitarleft@outline}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[guitar left, guitar left outline color=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@guitarleft{<guitar-left-color>}
\subkeyexplain{guitar left}{guitar left fret color}{color}{\pingu@color@guitarleft@fret}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[guitar left, guitar left fret color=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@guitarleft{<guitar-left-color>}
\subkeyexplain{guitar left}{guitar left fret side color}{color}{\pingu@color@guitarleft@fret@side}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[guitar left, guitar left fret side color=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@guitarleft{<guitar-left-color>}
\subkeyexplain{guitar left}{guitar left head color}{color}{\pingu@color@guitarleft@head}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[guitar left, guitar left head color=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@guitarleft{<guitar-left-color>}
\subkeyexplain{guitar left}{guitar left strings color}{color}{\pingu@color@guitarleft@strings}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[guitar left, guitar left strings color=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@guitarleft{<guitar-left-color>}
\subkeyexplain{guitar left}{guitar left peg color}{color}{\pingu@color@guitarleft@peg}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[guitar left, guitar left peg color=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@guitarleft{<guitar-left-color>}
\subkeyexplain{guitar left}{guitar left strings end color}{color}{\pingu@color@guitarleft@strings@end}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[guitar left, guitar left strings end color=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@guitarleft{<guitar-left-color>}
\subkeyexplain{guitar left}{guitar left pickup color}{color}{\pingu@color@guitarleft@pickup}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[guitar left, guitar left pickup color=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@guitarleft{<guitar-left-color>}
\subkeyexplain{guitar left}{guitar left fret lines color}{color}{\pingu@color@guitarleft@fret@lines}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[guitar left, guitar left fret lines color=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

\subkeyexplain{guitar left}{guitar left strings shift}{length}{\pingu@x@guitarleft@stringshift}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[guitar left, guitar left strings shift=2mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\showkeyexplain{guitar right}{color}{pingu@red!90!black}
\begin{tcblisting}{}
\begin{tikzpicture}
	\pingu[guitar right=green]
\end{tikzpicture}
\end{tcblisting}
\endshowkeyexplain

{\def\pingu@color@guitarright{<guitar-right-color>}
\subkeyexplain{guitar right}{guitar right side color}{color}{\pingu@color@guitarright@side}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[guitar right, guitar right side color=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@guitarright{<guitar-right-color>}
\subkeyexplain{guitar right}{guitar right tint color}{color}{\pingu@color@guitarright@tint}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[guitar right, guitar right tint color=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@guitarright{<guitar-right-color>}
\subkeyexplain{guitar right}{guitar right outline color}{color}{\pingu@color@guitarright@outline}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[guitar right, guitar right outline color=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@guitarright{<guitar-right-color>}
\subkeyexplain{guitar right}{guitar right fret color}{color}{\pingu@color@guitarright@fret}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[guitar right, guitar right fret color=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@guitarright{<guitar-right-color>}
\subkeyexplain{guitar right}{guitar right fret side color}{color}{\pingu@color@guitarright@fret@side}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[guitar right, guitar right fret side color=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@guitarright{<guitar-right-color>}
\subkeyexplain{guitar right}{guitar right head color}{color}{\pingu@color@guitarright@head}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[guitar right, guitar right head color=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@guitarright{<guitar-right-color>}
\subkeyexplain{guitar right}{guitar right strings color}{color}{\pingu@color@guitarright@strings}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[guitar right, guitar right strings color=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@guitarright{<guitar-right-color>}
\subkeyexplain{guitar right}{guitar right peg color}{color}{\pingu@color@guitarright@peg}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[guitar right, guitar right peg color=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@guitarright{<guitar-right-color>}
\subkeyexplain{guitar right}{guitar right strings end color}{color}{\pingu@color@guitarright@strings@end}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[guitar right, guitar right strings end color=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@guitarright{<guitar-right-color>}
\subkeyexplain{guitar right}{guitar right pickup color}{color}{\pingu@color@guitarright@pickup}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[guitar right, guitar right pickup color=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

{\def\pingu@color@guitarright{<guitar-right-color>}
\subkeyexplain{guitar right}{guitar right fret lines color}{color}{\pingu@color@guitarright@fret@lines}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[guitar right, guitar right fret lines color=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain}

\subkeyexplain{guitar right}{guitar right strings shift}{length}{\pingu@x@guitarright@stringshift}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[guitar right, guitar right strings shift=2mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain
...
\subsubsection{The drum stick}

\showkeyexplain{drum stick left}{color}{pingu@bronze!90!white}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[drum stick left=green]
\end{tikzpicture}
\end{tcblisting}
\endshowkeyexplain

\subkeyexplain{drum stick left}{drum stick left handle length}{length}{\pingu@drumstick@left@length}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[drum stick left, drum stick left handle length=2mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{drum stick left}{drum stick left handle shift}{length}{\pingu@drumstick@left@shift}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[drum stick left, drum stick left handle shift=2mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{drum stick left}{drum stick left xshift}{length}{\pingu@drumstick@left@xshift}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[drum stick left, drum stick left xshift=2mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{drum stick left}{drum stick left yshift}{length}{\pingu@drumstick@left@yshift}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[drum stick left, drum stick left yshift=2mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{drum stick left}{drum stick left handle}{color}{\pingu@color@drumstick@left@handle}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[drum stick left, drum stick left handle=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\showkeyexplain{drum stick right}{color}{pingu@bronze!90!white}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[drum stick right=green]
\end{tikzpicture}
\end{tcblisting}
\endshowkeyexplain

\subkeyexplain{drum stick right}{drum stick right handle length}{length}{\pingu@drumstick@right@length}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[drum stick right, drum stick right handle length=2mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{drum stick right}{drum stick right handle shift}{length}{\pingu@drumstick@right@shift}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[drum stick right, drum stick right handle shift=2mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{drum stick right}{drum stick right xshift}{length}{\pingu@drumstick@right@xshift}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[drum stick right, drum stick right xshift=2mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{drum stick right}{drum stick right yshift}{length}{\pingu@drumstick@right@yshift}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[drum stick right, drum stick right yshift=2mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{drum stick right}{drum stick right handle}{color}{\pingu@color@drumstick@right@handle}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[drum stick right, drum stick right handle=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subsubsection{The microphone}

\showkeyexplain{microphone left}{color}{pingu@silver!90!white}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[microphone left=green]
\end{tikzpicture}
\end{tcblisting}
\endshowkeyexplain

\subkeyexplain{microphone left}{microphone left handle length}{length}{\pingu@microphone@left@length}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[microphone left, microphone left handle length=2mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{microphone left}{microphone left handle shift}{length}{\pingu@microphone@left@shift}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[microphone left, microphone left handle shift=2mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{microphone left}{microphone left xshift}{length}{\pingu@microphone@left@xshift}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[microphone left, microphone left xshift=2mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{microphone left}{microphone left yshift}{length}{\pingu@microphone@left@yshift}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[microphone left, microphone left yshift=2mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{microphone left}{microphone left handle}{color}{\pingu@color@microphone@left@handle}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[microphone left, microphone left handle=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\showkeyexplain{microphone right}{color}{pingu@silver!90!white}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[microphone right=green]
\end{tikzpicture}
\end{tcblisting}
\endshowkeyexplain

\subkeyexplain{microphone right}{microphone right handle length}{length}{\pingu@microphone@right@length}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[microphone right, microphone right handle length=2mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{microphone right}{microphone right handle shift}{length}{\pingu@microphone@right@shift}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[microphone right, microphone right handle shift=2mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{microphone right}{microphone right xshift}{length}{\pingu@microphone@right@xshift}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[microphone right, microphone right xshift=2mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{microphone right}{microphone right yshift}{length}{\pingu@microphone@right@yshift}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[microphone right, microphone right yshift=2mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{microphone right}{microphone right handle}{color}{\pingu@color@microphone@right@handle}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[microphone right, microphone right handle=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain
...
\subsubsection{The sword}

\showkeyexplain{sword left}{color}{pingu@silver!94!blue}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[sword left=green]
\end{tikzpicture}
\end{tcblisting}
\endshowkeyexplain

\subkeyexplain{sword left}{sword left handle length}{length}{\pingu@sword@left@handle@length}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[sword left, sword left handle length=7mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{sword left}{sword left handle shift}{length}{\pingu@sword@left@shift}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[sword left, sword left handle shift=2mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{sword left}{sword left xshift}{length}{\pingu@sword@left@xshift}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[sword left, sword left xshift=2mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{sword left}{sword left yshift}{length}{\pingu@sword@left@yshift}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[sword left, sword left yshift=2mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{sword left}{sword left handle}{color}{\pingu@color@sword@left@handle}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[sword left, sword left handle=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

{\def\pingu@color@sword@left@handle{<sword left handle color>}
\subkeyexplain{sword left}{sword left guard}{color}{\pingu@color@sword@left@guard}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[sword left, sword left guard=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{sword left}{sword left shade}{color}{\pingu@color@sword@left@shade}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[sword left, sword left shade=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{sword left}{sword left shade opacity}{factor}{\pingu@sword@left@shade@opa}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[sword left, sword left shade opacity=.5]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain
}

\subkeyexplain{sword left}{sword left length}{length}{\pingu@sword@left@length}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[sword left, sword left yshift=4mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{sword left}{sword left width}{length}{\pingu@sword@left@width}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[sword left, sword left width=.25mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\showkeyexplain{sword right}{color}{pingu@silver!94!blue}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[sword right=green]
\end{tikzpicture}
\end{tcblisting}
\endshowkeyexplain

\subkeyexplain{sword right}{sword right handle length}{length}{\pingu@sword@right@handle@length}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[sword right, sword right handle length=7mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{sword right}{sword right handle shift}{length}{\pingu@sword@right@shift}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[sword right, sword right handle shift=2mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{sword right}{sword right xshift}{length}{\pingu@sword@right@xshift}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[sword right, sword right xshift=2mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{sword right}{sword right yshift}{length}{\pingu@sword@right@yshift}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[sword right, sword right yshift=2mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{sword right}{sword right handle}{color}{\pingu@color@sword@right@handle}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[sword right, sword right handle=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

{\def\pingu@color@sword@right@handle{<sword right handle color>}
\subkeyexplain{sword right}{sword right guard}{color}{\pingu@color@sword@right@guard}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[sword right, sword right guard=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{sword right}{sword right shade}{color}{\pingu@color@sword@right@shade}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[sword right, sword right shade=green]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{sword right}{sword right shade opacity}{factor}{\pingu@sword@right@shade@opa}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[sword right, sword right shade opacity=.5]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain
}

\subkeyexplain{sword right}{sword right length}{length}{\pingu@sword@right@length}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[sword right, sword right yshift=4mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain

\subkeyexplain{sword right}{sword right width}{length}{\pingu@sword@right@width}
\begin{tcblisting}{@}
\begin{tikzpicture}
	\pingu[sword right, sword right width=.25mm]
\end{tikzpicture}
\end{tcblisting}
\endsubkeyexplain
...
\subsection{Anchors and Placement}\label{internal:anchors}

\subsubsection{Important Commands}

\begin{commandexplain}{pingu@bbox@left}{}
	Together with \cmdref{pingu@bbox@right}, \cmdref{pingu@bbox@bottom}, \cmdref{pingu@bbox@top},
	\cmdref{pingu@bbox@midx}, and \cmdref{pingu@bbox@midy} this dimension spans the silhouette of
	the body, the box every anchor refers to.
\end{commandexplain}

\begin{commandexplain}{pingu@bbox@right}{}
	See \cmdref{pingu@bbox@left}.
\end{commandexplain}

\begin{commandexplain}{pingu@bbox@bottom}{}
	See \cmdref{pingu@bbox@left}.
\end{commandexplain}

\begin{commandexplain}{pingu@bbox@top}{}
	See \cmdref{pingu@bbox@left}.
\end{commandexplain}

\begin{commandexplain}{pingu@bbox@midx}{}
	See \cmdref{pingu@bbox@left}.
\end{commandexplain}

\begin{commandexplain}{pingu@bbox@midy}{}
	See \cmdref{pingu@bbox@left}.
\end{commandexplain}

\begin{commandexplain}{pingu@bbox@update}{}
	Recalculates the six dimensions of the silhouette. Only \keyref{height} affects them.
\end{commandexplain}

\begin{commandexplain}{pingu@anchor@add}{\mand{name}\mand{x}\mand{y}}
	Registers a new named anchor for \keyref{anchor}.
\end{commandexplain}

\begin{commandexplain}{pingu@place@anchors}{}
	Creates the outer coordinates (|center|, |north|, \ldots{}, and |anchor|) of the current penguin.
\end{commandexplain}

\subsubsection{The Code}
The silhouette and all named anchors:
\includecode{pingu@anchor}
Relative placement joins the two anchors facing each other:
\includecode{pingu@relative}
Both are hooked into these keys:
\includecode{anchor\ keys}

\subsection{The Bends}\label{internal:bends}
\lpingu{to[bend right=<angle>]} is expensive, so all body bends use precomputed control points:
\includecode{pingu@bend}
