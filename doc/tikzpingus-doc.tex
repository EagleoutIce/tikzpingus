...
\def\@CreateCodeHyperLink#1#2#3#4{\lstset{add to literate={#2}{{\hyperref[#3]{\optstyle#4}}}{#1}}}
...
\def\CreateCodeHyperLink#1#2{% \StrLen{\@elem}[\@cclen]
	...
	\edef\@tmp{\noexpand\@CreateCodeHyperLink{#2}{\@sub}{pk:/pingu/\@elem}{\@subb}}\@tmp\fi}%
}
...
\def\mand##1{\texttt{\{##1\}}}%
% #2 is the default
\def\opt##1##2{\texttt{\textit{[##1\ifx!##2!\else\textcolor{lightgray}{\smaller\sffamily=##2}\fi]}}}%
\def\@explainsuff{#2}%
\keyexplain[]{\\#1}{}{}%
}{\endkeyexplain\endgroup}
...
% TODO: make the default argument of optionals a optional argument :D
\begin{commandexplain}{pingu}{\opt{penguin keys}{}}
	The command to draw the penguin!
\end{commandexplain}

\begin{commandexplain}{pingu@name}{}
	Holds the name of the penguin. Do not redefine, but use \keyref{name}.
\end{commandexplain}
