...
\def\PenguinTitle{%
\setbox0=\hbox{\tikz{\pingu[body type=tilt-right,eyes wink,conical hat,cane right,bow tie=pingu@purple,left wing grab,cup=purple!72!pingu@black]}}%
...
\begin{center}
	...
	\typeout{measurepage: \measurepage}%
...
See \autoref{wing-showcase}.

\begin{tcblisting}{sidebyside=false, title=Wing-Showcase,externalize listing=wings-showcase,float,label=wing-showcase}
\begin{tikzpicture}
...
Furthermore, the convenient selectors |eyes <style>|, |left eye <style>|, and |right eye <style>| exist as well. See \autoref{eye-showcase}.

\begin{tcblisting}{sidebyside=false, title=Eye-Showcase,externalize listing=eyes-showcase,float,label=eye-showcase}
\begin{tikzpicture}
...
\end{itemize}
For each selection, \say{none} will prohibit the drawing, and \say{normal} is the default chosen. See \autoref{bodyparts-showcase}.
\begin{tcblisting}{sidebyside=false, title=Bodyparts-Showcase,externalize listing=bodyparts-showcase,float,label=bodyparts-showcase}
\begin{tikzpicture}
